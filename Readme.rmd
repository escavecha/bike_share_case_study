...
This fictional case study packet is based on the Divvy case study, "'Sophisticated, Clear, and Polished’: Divvy and Data Visualization" written by
Kevin Hartman. (https://artscience.blog/home/divvy-dataviz-case-study)
...
Use the given data set for 12-month period, analyze different patterns and present findings
and suggestions along with the result of analysis. R has used for this work.
...
set the working directory by `setwd()` command as necessary.
```{r}
...
Step 3. check for column names to quick review. To merge data frames into one,
column names and attributes should consistent each other.
```{r}
...
```
According to the values in a new column 'duration_min'.
Current 'tripduration' column indicates values in seconds. To avoid confusion, change the column name accordingly.
```{r}
...
```
I will remove redundant columns for further analysis.
Copy a new dataframe after remove `year` and `duration_sec` columns.
```{r}
...
Trips_2019v2 <- Trips_2019v2[, c("trip_id",
                                 ...
                                 "duration_min")]
```
So far, `usertype`column has not inspected yet.
Check for the occurrences of values in this column.
```{r}
table(Trips_2019v2$usertype)
```
There are 880,631 rows of **Customer** and 2,937,360 rows of **Subscriber**.
Update the terms to align with the terms (**Casual**, **Member**) in the scenario.
```{r}
...
Trips_2019v2$days <- ordered(Trips_2019v2$days, levels=c("Monday",
                                                         ...
                                                         "Sunday"))
```
The pre-processing part is not complete. In summary, following works will be done in this part:
...
table(Trips_2019v2$usertype)
```
The fraction of Casual ride is about 23% of total rides.
Then how these use patterns differ, compared to the that of members? Let's compare brief statistics between user type.
```{r}
...
# Create line chart
p1 <- ggplot(df_mean_duration,
             aes(x = days, y = mean_duration, group = usertype, color = usertype)) +
  geom_line() +
  geom_point() +
