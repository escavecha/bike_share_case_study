...
1. on average, _Casual_ users use the bike for around 29 to 41 minutes.
2. on average, _Members_ use the bike around 11 to 14 minutes.
3. average duration is the least in February for _Casual_ riders.
4. average duration is the least in November and December for _Members_.
5. Compared to _Casual_ riders, the variance in the trend is smaller for _Members_.
...
Consistency in the patterns may related to the commute purpose. To verify this hypothesis, continue to analyze the daily patterns for number or rides.
...
The pattern is in line with the hypothesis from Figure 3. We can assume that those rides are mostly used for commute for *Members*, while  rides are increased during weekends for *casual* riders use it for leisurely purpose on weekend. Still, *casual* riders are using the bike on the weekdays too.

Let's take a look on the patterns on hourly basis on each day to verify this finding. I will plot the charts for weekdays and weekends to see the difference.
...
2. Average duration for *Casual* riders are shortest in the morning time (at 6 to 7am) and peaks at 10am. Generally, *Casual* riders take a longer trip for their use.
...
From the patterns, it is more clear that main purpose of rides for *Members* is for commute. Unlike *Casual* riders patterns, *Members* have consistent patterns. To confirm, visulizae the same data for weekends.
...
p6a_weekend_mean <- weekend_data_mean %>%
  ...
  labs(x = 'Hour', 
       y = 'Avg. duration (min)',
       title = 'Fig 6a. Average Duration by Membership on Weekends, Hourly (2019)')
...
p6b_weekend_mean <- weekend_data_mean %>%
  ...
  labs(x = 'Hour', 
       y = 'No. of rides',
       title = 'Fig 5b. Number of Rides per Station by Membership on Weekend, Hourly (2019)') +
  annotate("text", x = 0, y = Inf,
           label = paste("Total number of stations:", n_unique_from_station_id),
           hjust = 0, vjust = 0.9)
