...
This fictional case study packet is based on the *Divvy* case study, "'Sophisticated, Clear, and Polished': Divvy and Data Visualization" written by Kevin Hartman ([link](https://artscience.blog/home/divvy-dataviz-case-study%3E)).
...
> other station in the system. *Divvy* is a program of the Chicago Department of Transportation (CDOT),
> which owns the city's bikes, stations and vehicles.([Link](https://divvybikes.com/about))
...
The data has been made available or case study by *Motivate International Inc.* under this license
...
I chose `Divvy_Trips_2019_Q1.zip` to `Divvy_Trips_2019_Q4.zip` files to use. set the working directory by `setwd()`
...
The Minimum value in duration_min is -56.37. Since this column has calculated based on the time difference between
`end_time` and `start_time`, negative value seems strange. The packet explains that those values are from the bikes are
taken out of services. So I will delete those rows from the data.

```{r remove false duration}
Trips_2019v1 <- subset(Trips_2019v1, duration_min >= 0)
```

Now updated dataframe has 3,817,991 rows, which 13 rows are removed - It won't affect much for the analysis.
...
Compared to the description in scenario, the size of fleet has increased, while number of stations has decreased.
...
Majority of the rides are made by *members* (about 77% of total rides). Assume current fleet size is maintained, it
is our interest how to attract the *casual* users to become a *member*. Let's compare statistics for riding times in
...
Maximum and minimum values in `duration_min` seem too extreme. However, the mean value suggests difference between
users: *Casual* users ride longer than *Members* per ride, even though their portion of total usage is less than 25%. On
the contrary, *members* ride for short distance, but more frequently. \# TODO How to deal with extreme values?
...
> NOTE TODO there are several winter bike events in February. I suspect that those events are reflected to the usage of
> casual users, but couldn't find the detail. related links are here.

We can assume that mentioned February events are factored in the patterns. or was it? To see the patterns in different
angle, this time plot the data for number of riders for month.
...
the least rides are made in February, while the most rides are made in August - Which is seasonal.

> Key finding: ride usage are really depends on the weather condition. people use less during winter, and more during
> summer. We can schedule the annual maintenance plan of the fleets focused on during winter time to allow more bike
> use. The peak in duration in February seems to be made by casual riders - but without detailed information of events
> it is presumed, not confirmed.

#### Step 3. Daily patterns

Let's take a look in detail for daily patterns. Let's plot a line chart for daily usage patterns.
