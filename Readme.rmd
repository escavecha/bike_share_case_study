...
By the time I have complete this case study, *Cyclistic*'s historical trip data from 2013 to July 2023 has provided for
analysis: ([link](https://divvy-tripdata.s3.amazonaws.com/index.html))
...
# install.packages("lubricate", repos = "https://cran.us.r-project.org")
...
There are two things noticed from `duration_min`. First, The Minimum value in duration_min is -56.37. Since this column
has calculated based on the time difference between `end_time` and `start_time`, negative value seems strange. The
packet explains that those values are from the bikes are taken out of services. So I will delete those rows from the
data.
...
Second, Compared to Mean and 3rd Quarterly value, the Max value is far too extreme. Meaning that the bike has turned in
177200 minutes later (or 123 days) since `start_time`. How to handle those cases?

In the real-world, *Divvy* mentions in FAQ as below: \> (Quote) Do not under any circumstances leave your bike
unattended; if you left your bike outside of a station please return immediately and re-dock your bike. Please know that
bikes missing for longer than 24 hours can result in a \$1,200 fee (plus tax) charged to the account holder that took
out the bike ([link](https://help.divvybikes.com/hc/en-us/articles/360033123412-My-bike-was-lost-or-stolen)).

Thus, I will assume that the data with `duration_min` is more than 24 hours (1440 min) is abnormal instances, and also
exclude from the scope of analysis. Let's check how many rows fit in this criteria.
...
Compared to the description in scenario, the fleet size has expanded from 5824 to 6017, while number of stations has
decreased from 692 to 616, since 2016.
...
the contrary, *members* ride for short distance, but more frequently. To identify the detail patterns in this
difference, I will start to break those patters from monthly to hourly scale in following analysis.
...
To see the patterns in different angle, plot the data for number of riders for monthly. This time I will use the average
...
2.  Ride usage are similar to the monthly weather condition. people rides less during winter, while rides more during
    summer.

Recommendation: We can schedule the annual maintenance plan of the fleets focused on during winter time to allow more
...
Consistency in the patterns may related to the commute purpose. To verify this hypothesis, continue to analyze the daily
patterns for number or rides.
...
1.  Average number of rides for *Members* are at highest level along the weekdays. On weekends, those numbers drop in
    half.
2.  On the contrary, the average number or rides are highest on Saturday for *Casual* riders.

The pattern is in line with the hypothesis from Figure 3. We can assume that those rides are mostly used for commute for
*Members*, while rides are increased during weekends for *casual* riders use it for leisurely purpose on weekend. Still,
*casual* riders are using the bike on the weekdays too.
...
ggsave("p4_weekdays.png", plot = p5_weekdays_mean)
```

Key findings:

1.  Average duration for *Members* are stable in the range of 10 to 15 minutes.
2.  Average duration for *Casual* riders are shortest in the morning time (at 6 to 7am) and peaks at 10am. Generally,
    *Casual* riders take a longer trip for their use.
...
From the patterns, it is more clear that main purpose of rides for *Members* is for commute. Unlike *Casual* riders
patterns, *Members* have consistent patterns. To confirm, visualize the same data for weekends.
...
ggsave("p6_weekend.png", plot = p6_weekend_mean)
```
...
From Figure 5b and 6b, the least busiest hours are from midnight to 4am for everyday. Therefore if any maintenance or
the relocation of the bikes should be made, this will be the most optimum window. If night shift is unavailable, then
the 4 hours window from 10am to 2pm on weekdays will be alternative to schedule.
...
1.  Considering the monthly ride patterns, schedule the annual maintenance plan of the fleets focused on during winter
    time to allow more availability of the bikes for rest of the seasons.
2.  Schedule daily services & maintenance on the fleets at midnight to 4am (daily), or 10am to 2pm (weekdays).

### Next steps

From Figure 5b, the most busiest times are around 8am and 5pm. To attract people to subscribe membership, adequate number of bikes should be prepared at the stations. As the number of bikes in the fleets is limited, We might think to relocated the bikes from the least busiest stations to most busiest stations to accommodate most usage. Further analysis will be focused on to identify the loads on the stations, and figure out how to provide flexibility of the fleets across the stations. 
